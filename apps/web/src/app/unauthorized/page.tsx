...
export default function UnauthorizedPage(): React.ReactNode {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-zinc-950 p-8 text-center">
      <h1 className="text-2xl font-semibold tracking-tight text-zinc-100">Access denied</h1>
      ...
      {/* Not the Button primitive: an <a> must never wrap a <button> (invalid HTML,
          double-interactive for a11y) — style the link directly instead. */}
      <Link
        href="/reports"
        className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-zinc-100 transition-colors hover:bg-white/10"
      >
        Back to my reports
      </Link>
    </main>

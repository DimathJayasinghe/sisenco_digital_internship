...
export function HistoryList(): ReactNode {
  ...
  if (isLoading) {
    return <p className="mt-8 text-sm text-zinc-400">Loading…</p>;
  }

  if (isError) {
    return <p className="mt-8 text-sm text-red-400">Couldn&apos;t load your report history.</p>;
  }

  if (!reports || reports.length === 0) {
    return <p className="mt-8 text-sm text-zinc-400">No reports yet.</p>;
  }

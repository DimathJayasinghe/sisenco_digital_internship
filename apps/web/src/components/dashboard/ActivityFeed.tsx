...
export function ActivityFeed({ data }: ActivityFeedProps): ReactNode {
  return (
    <Card className="p-4">
      ...
      {data.length === 0 ? (
        <p className="mt-4 text-sm text-zinc-400">No submissions yet.</p>
      ) : (
            ...
            <li key={report.id} className="flex items-center justify-between gap-4 py-2.5 text-sm">
              <div className="min-w-0">
                <p className="truncate text-zinc-200">
                  ...
                  <span className="text-zinc-400"> · {report.project.name}</span>
                </p>
                <p className="text-xs text-zinc-400">{formatWeekRange(report.weekStartDate)}</p>
              </div>

...
- [ ] `JwtStrategy` (passport-jwt) — reads token from the `HttpOnly` cookie, validates, returns `AuthUser`
- [ ] `JwtAuthGuard` — global guard, respects `@Public()` to bypass
- [ ] `RegisterDto` / `LoginDto` with class-validator decorators
- [ ] `AuthService` — register (always `TEAM_MEMBER`, bcrypt hash ≥12 rounds), login (bcrypt compare, generic error), issue JWT
- [ ] `AuthController` — `POST /auth/register`, `POST /auth/login` (sets `HttpOnly; Secure; SameSite=Strict` cookie), `POST /auth/logout` (clears cookie), `GET /auth/me`
- [ ] Wire `JwtAuthGuard` as a global `APP_GUARD` in `AppModule`
- [ ] Strict throttling on `/auth/login` and `/auth/register` (10 req/60s)
